...
### إصلاح خطأ تغيير نوع العقد (ReferenceError - driverExpenses/driverRevenues)

**المشكلة:** عند تغيير نوع عقد السائق (من شهري إلى يومي أو العكس)، كانت تظهر رسالة "حدث خطأ في حفظ البيانات".

**السبب الجذري:** في دالة الحفظ (submit handler)، كان المتغيران `driverExpenses` و `driverRevenues` يُستخدمان بدون تعريف (`const/let/var`)، مما يسبب `ReferenceError` يمنع تنفيذ الكود ويُمسكه الـ `catch` ليظهر رسالة الخطأ.

**الإصلاح:** إضافة `const` للمتغيرين في السطرين 840 و 847 من `drivers.html`.

**الملف المعدل:** `drivers.html`
...
### إصلاح: مشكلة الأرشفة عند تغيير نوع العقد

**المشاكل التي تم إصلاحها:**
1. الأرشيف كان يُحفظ بمعرف عشوائي (`addDoc`) بدلاً من الرقم الوظيفي
2. البيانات الجديدة في صفحة الإدارة كانت تُحفظ بدون رقم وظيفي أحياناً
3. منطق منع التكرار كان يستخدم رقم الهاتف بدلاً من الرقم الوظيفي

**الإصلاح في `drivers.html`:**
- تغيير `addDoc` إلى `setDoc` مع معرف تسلسلي: `DRV001_archive_1`, `DRV001_archive_2`, ...
- إضافة `employeeNumber` صريحاً في بيانات الأرشيف
- تغيير منطق منع التكرار ليستخدم `employeeNumber` بدلاً من `phone`
- تأمين `employeeNumber` و `driverId` عند تحديث بيانات السائق

**الملفات المعدلة:** `drivers.html`, `sw.js`
